...
proc randVector*[N: static[int]](max_float=1.0): Vector[N] =
    for i in countup(0, N-1):
        result.elements[i] = rand(max_float)

proc Vector1*(elements: varargs[float64, float64]): Vector[1] =
    newVector[1](elements)
proc Vector2*(elements: varargs[float64, float64]): Vector[2] =
    newVector[2](elements)
proc Vector3*(elements: varargs[float64, float64]): Vector[3] =
    newVector[3](elements)
proc Vector4*(elements: varargs[float64, float64]): Vector[4] =
...
proc `[]`*[I: Ordinal](v: Vector, i: I): float64 =
    v.elements[i]
proc `[]=`*[I: Ordinal](v: var Vector, i: I, x: float64) =
    v.elements[i] = x
